...
export default function useShortcuts() {
    ...
    if (!['input', 'textarea'].includes(target.tagName.toLowerCase())) {
      ...
      if (ev.ctrlKey) {
        ...
        if (key === 'arrowup') { // 组件面板
          toggleCompsPanel()
          ev.preventDefault()
        } else if (key === 'arrowdown') { // 配置面板
          toggleToolbox()
          ev.preventDefault()
        } else if (key === 'arrowleft') { // 图层
          toggleLayerPanel()
          ev.preventDefault()
        } else if (key === 'arrowright') { // 配置面板
          toggleConfigPanel()
          ev.preventDefault()
        } else if (key === 'a') { // 最佳视窗缩放
          autoCanvasScale()
          ev.preventDefault()
        } else if (key === 'd') { // 100%视角
          setCanvasScale(100)
          ev.preventDefault()
        } else if (key === 'c') { // 缓存复制组件
          copyTempCom = selectedCom.value
          ev.preventDefault()
        } else if (key === 'v') { // 缓存复制组件
          copyTempCom && copyCom(copyTempCom.id)
          ev.preventDefault()
        } else if (key === 's') { // 保存当前面板
          headerRef.value && headerRef.value.handleSaveScreen()
          ev.preventDefault()
        } else if (key === 'z') { // 撤销
          store.dispatch('schema/undo')
          ev.preventDefault()
        }
      }
      if (ev.shiftKey) {
  ...
  const keyDown = (key) => {
    store.dispatch('schema/toggleKey', { key, status: true })
  }

  const keyUpEvent = () => {
    store.dispatch('schema/resetKeyCode')
  }
