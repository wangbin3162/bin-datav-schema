...
const style = computed(() => {
  return {
    marginTop: props.top,
    width: props.width,
    left: `${document.documentElement.clientWidth - 348 * 2 - 24}px`,
  }
})
const bodyStyle = computed(() => {
  const height = Math.max(props.maxBodyHeight, document.documentElement.clientHeight - 170)
  return {
    maxHeight: `${height}px`,
    overflow: 'auto',
  }
...
.modal-config {
  ...
  &-header {
    ...
    &:after {
      ...
      bottom: 0;
      height: 1px;
      background-color: var(--schema-color-border);
    }
    &:before {
      content: '';
      position: absolute;
      ...
      background-image: linear-gradient(
        270deg,
        var(--bin-color-primary) 0,
        var(--bin-color-primary-light3) 74%
      );
    }
    .b-iconfont {
      cursor: pointer;
