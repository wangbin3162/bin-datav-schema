...
const getSeries = yData => {
  ...
  return yData.map((item, index) => {
    const cfg = {
      ...
      barGap: `${global.barGap}%`,
      barCategoryGap: `${global.barCategoryGap}%`,
      showBackground: global.showBackground,
      backgroundStyle: global.backgroundStyle,
      itemStyle: { borderRadius: [...global.itemStyle.borderRadius] },
      data: item.data,
    }
    if (global.itemStyle.type === 'gradient') {
      const currentColor = themeColors.value[index]
      const from = ColorHelper.alpha(currentColor, global.itemStyle.from * 0.01)
      const to = ColorHelper.alpha(currentColor, global.itemStyle.to * 0.01)
      cfg.itemStyle.color = {
        type: 'linear',
        x: 0,
        y: 0,
