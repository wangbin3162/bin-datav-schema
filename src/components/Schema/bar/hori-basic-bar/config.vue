<template>
  <div class="setting-panel-gui">
    ...
    <g-field-collapse label="柱子标注" modal toggle v-model="config.label.show">
      <g-field label="位置">
    ...
    <g-field-collapse label="Y轴" modal toggle v-model="config.yAxis.show">
      <g-field label="类型">
        <b-radio-group v-model="config.yAxis.type" type="button" size="mini">
          <b-radio v-for="em in yAxisTypes" :key="em.value" :label="em.value">
            {{ em.label }}
          </b-radio>
        </b-radio-group>
      </g-field>
      ...
      <g-field-collapse label="轴标签" v-model="config.yAxis.axisLabel.show" toggle>
        ...
        <g-field label="两端留白">
          <div class="pt-5">
            <b-switch v-model="config.yAxis.boundaryGap" size="small" />
          </div>
        </g-field>
        <g-field
          label="间隔"
        ...
        >
          <g-input v-model="config.yAxis.axisLabel.interval" />
        </g-field>
        <g-field label="展示方式" flat>
          <g-input-number
    ...
    <g-field-collapse label="X轴" modal toggle v-model="config.xAxis.show">
      <g-field label="显示范围" flat tooltip="默认可以填写auto，或者自行输入数值">
        <g-input v-model="config.xAxis.extent.min" inline label="最小值" />
        <g-input v-model="config.xAxis.extent.max" inline label="最大值" />
      </g-field>
      ...
      >
        <g-input-number v-model="config.xAxis.splitNumber" :min="0" :max="100" :step="1" />
      </g-field>
