...
export class NavCanvas {
  ...
  createPattern(canvas) {
    const [R, G, B] = Utils.color.toRgbArr(this.hightlightColor)
  ...
  drawHightlight(index) {
    ...
    gradient.addColorStop(Math.min(1, Math.max(0, 0 + f + 0.1)), this.hightlightColor)
    gradient.addColorStop(Math.min(1, 0 + f + d), this.hightlightColor)
