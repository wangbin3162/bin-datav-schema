...
const showLine = (isDownward, isRightward) => {
  ...
  components.forEach(component => {
    ...
    const { top, left, bottom, right, width, height } = getComponentRotatedStyle(
      compStyle(component.attr),
    )
...
onMounted(() => {
  EventBus.on(EventMap.CompMove, ({ isDownward, isRightward }) => {
    if (toolbox.value.markLine) {
      showLine(isDownward, isRightward)
    }
  })
  EventBus.on(EventMap.CompUnMove, () => {
    hideLine()
  })
})
onBeforeUnmount(() => {
  EventBus.off(EventMap.CompMove)
  EventBus.off(EventMap.CompUnMove)
})
</script>
