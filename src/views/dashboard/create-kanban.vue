<template>
  ...
  >
    <div>
      ...
      <b-empty v-show="templates.length === 0">暂无模板</b-empty>
    </div>
    <template #footer>
      <b-button @click="closeModal">取消</b-button>
      <b-button type="primary" @click="createScreen">开始设计</b-button>
    </template>
  </b-modal>
</template>
...
.template-list {
  ...
  .template-item {
    ...
    .template-size {
      ...
      color: var(--s-text-color-2);
      text-align: right;
      font-family: Orbitron-Bold, PingFangSC-Medium, PingFang SC, Microsoft YaHei, Arial, Helvetica,
        sans-serif;
    }
    &.selected {
      border: 1px solid var(--bin-color-primary);
